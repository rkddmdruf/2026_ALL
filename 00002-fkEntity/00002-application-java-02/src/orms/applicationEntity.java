...
public class applicationEntity {
	...
	public static void reload() {
		cache.clear();
		try (var stmt = DBManager.execute("SELECT * FROM application"); var rs = stmt.executeQuery()) {
			while (rs.next()) {
	...
	public void save() {
		String sql = (a_no == null) ? "INSERT INTO application (u_no,j_no,a_date,a_state) VALUES (?,?,?,?)"
				: "UPDATE application SET u_no = ?,j_no = ?,a_date = ?,a_state = ? WHERE a_no = ?";
		Object[] values = (a_no == null) ? new Object[] { u_no, j_no, a_date, a_state }
	...
	public jobEntity getjobEntity() {
		return jobEntity.findById(j_no).get();
	}
	...
	public String stateText() {
		return a_state == 0 ? "대기" : a_state == 1 ? "합격" : "불합격";
	}
}

...
public class MyPage extends CFrame{
	...
	protected void design() {
		...
		JPanel name_exceptionPanel = new BoxPanel(BoxPanel.C, 0, 20, 0, userPanel(), myLetter(),
					scrollPanel1 = jobPanel("내가 지원한 알바", user.getapplicationEntity()), 
					scrollPanel2 = jobPanel("관심 알바 목록", user.getfavoriteEntity())
    ...
    private JPanel userPanel() {
		...
		return new BoxPanel(BoxPanel.C, 0, 10, 0, IntStream.range(0, strs.length).mapToObj(c -> {
			...
			if(c == strs.length - 1) {
				...
				return new BoxPanel(BoxPanel.R, 0, 0, 0, BoxPanel.fill(locationPanel));
			}
			return new BoxPanel(BoxPanel.R, 0, 0, 0, label, BoxPanel.HGAP(), BoxPanel.fill(tfs.get(c + 1))).setBackColor(Color.white); // 시작이 name이라서 +1
    ...
    private JScrollPane getCustomScrollPane(Object obj) {
    	...
    	((List<Object>) obj).forEach(e -> {
    		jobEntity job = null;
    		if(e instanceof applicationEntity) job = ((applicationEntity ) e).getjobEntity();
    		else if(e instanceof favoriteEntity) job = ((favoriteEntity) e).getjobEntity();
    		else throw new RuntimeException("파라미터로 온 값이 잘못되었거나, null입니다.");
    		
    		storeEntity store = job.getstoreEntity();

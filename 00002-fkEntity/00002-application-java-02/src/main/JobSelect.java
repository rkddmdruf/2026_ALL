...
public class JobSelect extends CFrame{
	...
	public JobSelect(int s_no) {
		store = storeEntity.findById(s_no).get();
	...
	private JScrollPane setMainPanel() {
		List<jobEntity> list = store.getjobEntity();//e -> e.s_no == s_no
	...
	private JPanel jobPanel(jobEntity j) {
		...
		JLabel wLabel = new JLabel(store.getcategoryEntity().c_name, JLabel.CENTER);
		...
		JLabel subLabel = new JLabel(store.getlocationEntity().l_name + " · " + j.j_start + " ~ " + j.j_end);

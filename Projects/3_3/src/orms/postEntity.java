...
public class postEntity {
	...
	public static void reload() {
		cache.clear();
		try (var stmt = DBManager.execute("SELECT * from post"); var rs = stmt.executeQuery()) {
			while (rs.next()) {
	...
	public String toString() {
		return "postEntity [p_no=" + p_no + ", u_no=" + u_no + ", p_content=" + p_content + ", p_files=" + p_files
				+ ", p_like=" + p_like + ", p_date=" + p_date + "]";
	}
	...
	public void save() {
		String sql = (p_no == null) ? "insert into post (u_no,p_content,p_files,p_like,p_date) values (?,?,?,?,?)"
				: "update post set u_no = ?,p_content = ?,p_files = ?,p_like = ?,p_date = ? where p_no = ?";
		Object[] values = (p_no == null) ? new Object[] { u_no, p_content, p_files, p_like, p_date }

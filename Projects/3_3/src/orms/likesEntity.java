...
public class likesEntity {
	...
	public static void reload() {
		cache.clear();
		try (var stmt = DBManager.execute("SELECT * from likes"); var rs = stmt.executeQuery()) {
			while (rs.next()) {
	...
	public void save() {
		String sql = (l_no == null) ? "insert into likes (p_no,u_no,l_date) values (?,?,?)"
				: "update likes set p_no = ?,u_no = ?,l_date = ? where l_no = ?";
		Object[] values = (l_no == null) ? new Object[] { p_no, u_no, l_date }

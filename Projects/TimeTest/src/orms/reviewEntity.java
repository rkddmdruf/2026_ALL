...
public class reviewEntity {
	...
	public static Optional<reviewEntity> findByFrist(Predicate<reviewEntity> f) {
		return findAll().stream().filter(f).findFirst();
	}
	
	public static boolean exist(Predicate<reviewEntity> f) {
		return !findBy(f).isEmpty();
	}

	public void save() {
		String sql = (rno == null) ? "insert into review set ono = ?,review = ?,star = ?)"

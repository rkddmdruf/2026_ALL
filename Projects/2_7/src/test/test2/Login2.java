...
public class Login2 extends CFrame {
	...
	JButton b1 = set(new JButton("로그인") {
		protected void paintComponent(Graphics g) {
			...
			if (getModel().isArmed())
				g2.setColor(b1.getBackground().brighter()); // 클릭 시
			else
				g2.setColor(b1.getBackground()); // 기본색
			g2.fillRoundRect(0, 0, getWidth(), getHeight(), 20, 20);
			super.paintComponent(g);
		};
	...
	public void desing() {
			...
			protected void paintComponent(Graphics g) {
				...
				for (Shape s : map.values()) {
					...
					if(mouseMoveNumber == n) {
						...
						g2.setColor(new Color(Math.min(255, c.getRed() + 20), Math.min(255, c.getGreen() + 20), Math.min(255, c.getBlue() + 20)));
					}
					g2.fill(s);
					...
					g2.drawString(strs.get(n), (float) r.getCenterX() - fm.stringWidth(strs.get(n++)) / 2, (float) r.getCenterY());
				}
		...
		p3.add(loginPanel());
		JLabel l1 = lb("← 이전");
		JPanel p = col(0, fw(row(0, l1).setBackColor(sp.imgBackColor)), f(set(p3, BG(sp.imgBackColor)))).setBackColor(sp.imgBackColor);
	...
	private JPanel loginPanel() {
		JPanel p = new JPanel(new BorderLayout()) {
			...
			protected void paintComponent(Graphics g) {
				...
				GradientPaint gd = new GradientPaint(0, 0, Util.setA(colors.get(mouseClickNumber), 240), 0, getHeight()-1, Util.setA(Color.white, 100));
				g2.setPaint(gd);
				g2.fillRoundRect(0, 0, getWidth(), getHeight(), 20, 20);
				g2.fillRoundRect(0, 0, getWidth(), getHeight(), 20, 20);

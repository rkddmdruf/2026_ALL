...
public class metro extends CFrame{
	...
	public metro(String start, String end) {
		...
		setFrame("경로 검색", 550, 800, () -> { thread.interrupt();  new Main3().setVisible(true);});
	...
	protected void desing() {
		label = new JLabel() {
			...
			protected void paintComponent(Graphics g) {
		        ...
		        g2.drawString("초기화", getWidth() - 10 - fm.stringWidth("초기화"), 0 + 10 + fm.getHeight());
		        stringPoint = new Rectangle(getWidth() - 10 - fm.stringWidth("초기화"), 0 + 10, fm.stringWidth("초기화"), fm.getHeight());
		        ...
		        if(!bfs.isEmpty() && stationNumber < bfs.size() - 1) {
		            ...
		            g2.translate(x, y);
		            g2.rotate(Math.atan2(s2.y - s1.y, s2.x - s1.x) + Math.PI / 2);
	...
	public List<Integer> dijkstar(int start, int end) {
		...
		for(int i : new int[] {1, 2, 7}) {
			...
			for(int s = 0; s < list.size(); s ++) {
				...
				for(stationEntity n : ns) {
					map.computeIfAbsent(nowStation.sno, k -> new ArrayList<>()).add(new int[] {
							n.sno, (int) Math.sqrt(Math.pow(nowStation.x - n.x, 2) + Math.pow(nowStation.y - n.y, 2))
					});
				}
			}
		}
		for(String s : "석남,부평구청,인천시청".split(",")) {
			...
			map.computeIfAbsent(ss.get(0).sno, k -> new ArrayList<>()).add(new int[] {ss.get(1).sno, (int) Math.sqrt(Math.pow(ss.get(0).x - ss.get(1).x, 2) + Math.pow(ss.get(0).y - ss.get(1).y, 2))});
			map.computeIfAbsent(ss.get(1).sno, k -> new ArrayList<>()).add(new int[] {ss.get(0).sno, (int) Math.sqrt(Math.pow(ss.get(0).x - ss.get(1).x, 2) + Math.pow(ss.get(0).y - ss.get(1).y, 2))});
		}
	...
	protected void action() {
		label.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				repaint();
				if(stringPoint.contains(e.getPoint())) {
					bfs = new ArrayList<>();
					start = -1; end = -1; step = 0; stationNumber = 0;
					bLabel.setText("역을 우클릭하여 출발역을 선택하세요");
					thread.interrupt();
				}
		...
		bLabel.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if(bLabel.getForeground() == Color.yellow) {

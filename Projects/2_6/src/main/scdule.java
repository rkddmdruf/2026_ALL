...
public class scdule extends CFrame{
	...
	private void setPanel() {
		...
		while(!firstTime.isAfter(endTime)) {
			...
			p.add(lb(firstTime.format(DateTimeFormatter.ofPattern("HH:mm")),FONT(sp.font.deriveFont(17f).deriveFont(1)), FG(sp.color)));
			p.add(lb("→", FG(sp.color), HOA(JLabel.CENTER)));
			p.add(lb(firstTime.plusMinutes(totalTime).format(DateTimeFormatter.ofPattern("HH:mm")),FONT(sp.font.deriveFont(17f).deriveFont(1)), FG(sp.color)));

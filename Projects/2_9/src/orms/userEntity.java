...
public class userEntity {
	...
	public static void reload() {
		...
		try (var stmt = DBManager.execute("SELECT * from user"); var rs = stmt.executeQuery()) {
			...
			sp.err(e.getLocalizedMessage() + "userEntity");
		}
	}

	@Override
	public String toString() {
		return "userEntity [uno=" + uno + ", id=" + id + ", pw=" + pw + ", uname=" + uname + ", point=" + point
				+ ", sno=" + sno + ", chance=" + chance + "]";
	}
	...
	public void save() {
		String sql = (uno == null) ? "insert into user (id,pw,uname,point,sno,chance) values (?,?,?,?,?,?)"
				: "update user set id = ?,pw = ?,uname = ?,point = ?,sno = ?,chance = ? where uno = ?";
		Object[] values = (uno == null) ? new Object[] { id, pw, uname, point, sno, chance }
